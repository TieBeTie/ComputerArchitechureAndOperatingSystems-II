...
path GetRealPathAndLastTime(const path &relative_path) {
  ...
  for (path &dir : in_dirs) {
    ...
    if (exists(absolute_path)) {
      if (last_write_time(absolute_path) > last_time || !is_exist) {
        is_exist = true;
...
int GetAttrCallback(const char *relative_path, struct stat *stbuf, struct fuse_file_info *fi) {
  (void) fi;

  stat(absolute(GetRealPathAndLastTime(relative_path + 1)).c_str(), stbuf);
...
int ReadDirCallback(
  ...
  for (path &dir : in_dirs) {
    ...
    if (exists(absolute_path) && is_directory(directory_entry(absolute_path).status())) {
      // перебираем свойства директорий
      for (const directory_entry &properties : directory_iterator(absolute_path)) {
        // кастуем к обычной си строке и добавляем в список файлов количестве 1
...
int ReadCallback(const char *relative_path,
                 char *buf,
                 size_t size,
                 off_t offset,
                 struct fuse_file_info *fi) {
  ...
  path real_path = GetRealPathAndLastTime(relative_path + 1);
