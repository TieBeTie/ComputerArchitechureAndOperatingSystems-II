...
int main(int argc, char **argv) {
  ...
  // The next 8-byte is the salt, which is exactly the same as openssl -p output.
  ...
  EVP_BytesToKey(EVP_aes_256_cbc(), EVP_sha256(), salt, password, strlen(password), 1, key, iv);
  ...
  while ((encoded_block_size = read(0, encoded, sizeof(encoded))) > 0) {
    EVP_DecryptUpdate(ctx, decoded, &decoded_block_size, encoded, encoded_block_size);
